...
function getDayName(date) {
  const result = new Date(date);
  return result.toLocaleDateString('en', { weekday: 'long' });
}
...
function getNextFriday(date) {
  ...
  const nextFriday = (5 - currentDay + 7) % 7;
  result.setDate(result.getDate() + (nextFriday === 0 ? 7 : nextFriday));
  return result;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const result = Math.round((endDay - startDay) / (1000 * 60 * 60 * 24)) + 1;
  return result;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= result.getDate(); i += 1) {
...
function getNextFridayThe13th(date) {
  ...
  let result = null;
  while (isFinding) {
    const currentMonth = new Date(year, month, 13);
